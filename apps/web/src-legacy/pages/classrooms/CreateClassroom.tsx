    ...
    async (e?: React.FormEvent) => {
      ...
      await wrap(async () => {
        ...
        if (gradeAndSection.trim()) {
          const normalizedGradeSection = gradeAndSection.trim().toUpperCase();
          const validation = validateGradeAndSection(normalizedGradeSection);
          if (!validation.valid) {
            showMsgRef.current(
              validation.error || "Invalid grade and section",
              "error",
            );
            return;
          }

          const parts = normalizedGradeSection.split("-");
          if (parts.length === 3) {
            grade = parts[0];
