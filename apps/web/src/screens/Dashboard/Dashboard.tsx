...
const Dashboard: React.FC = (): React.ReactElement => {
  ...
  const [showcaseItems] = useState<ShowcaseItem[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      safeStorageRemove(getSessionStorage(), "justLoggedIn");

      // Check global auth state first
      const globalAuth = getGlobalAuthState();

      const cachedUser = safeStorageGet(getLocalStorage(), "user");
      if (cachedUser) {
        try {
  ...
  const saveMySection = async (): Promise<void> => {
    ...
    try {
      ...
      if (data?.success) {
        setMySection(value);
        const stored = safeStorageGet(getLocalStorage(), "user");
        try {
          const parsed = stored ? JSON.parse(stored) : {};
          const merged = { ...parsed, section: value };
          safeStorageSet(getLocalStorage(), "user", JSON.stringify(merged));
