...
const Login: React.FC = (): React.ReactElement => {
  ...
  // Start null so server and client render identical HTML; hydrated in useEffect below.
  ...
  // Hydrate role from storage after mount and validate it.
  useEffect(() => {
    const loaded = localStorageGet({ keys: ["role"] })[0] as Role | null;
    setRole(loaded);
    if (!loaded || !valid_roles.includes(loaded)) {
  ...
  const handleLogin = useCallback(async (): Promise<void> => {
    await wrap(async () => {
      ...
      try {
        ...
        try {
          const { getSessionStorage, safeStorageSet } =
            await import("utils/safeStorage");
          safeStorageSet(getSessionStorage(), "justLoggedIn", "1");
        } catch {
