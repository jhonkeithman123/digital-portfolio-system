...
const DB_PROVIDER = (process.env.DB_PROVIDER || "mysql").toLowerCase();
const SUPABASE_ONLY =
  process.env.SUPABASE_ONLY === "true" || DB_PROVIDER === "supabase";
...
>(
  ...
  if (SUPABASE_ONLY) {
    throw Object.assign(
      new Error("MySQL query blocked: server is running in Supabase-only mode"),
      { code: "DB_DISABLED" },
    );
  }
...
async function connectLoop() {
  if (SUPABASE_ONLY) {
    console.log("DB connect loop skipped: Supabase-only mode enabled");
    return;
  }
...
export default {
  ...
  query,
  isDbAvailable,
  isSupabaseOnlyMode: () => SUPABASE_ONLY,
