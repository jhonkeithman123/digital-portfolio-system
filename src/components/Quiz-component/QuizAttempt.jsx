...
export default function QuizAttempt() {
  ...
  const { classCode: classroomCode, quizId } = useParams();
  ...
  const loadQuiz = useCallback(async () => {
    ...
    try {
      ...
      const pages = normalizePages(data.quiz?.questions?.pages);
      const attemptsRemaining = data.quiz?.attempts_remaining;
      const attemptsUsed = data.quiz?.attempts_used;
      ...
      if (attemptsRemaining === 0) {
        showMsgRef.current("No attempts remaining for this quiz.", "warning");
      }
    } catch (err) {
  ...
  async function submitAttempt() {
    ...
    if (data?.success) {
      ...
      setTimeout(() => {
        navigate(`/quizzes/${classroomCode}/quizzes/${quizId}/attempts`);
      }, 600);
    } else {
      showMsgRef.current(
        ...
        <div className="quiz-shell">
          <section className="quiz-attempt-card error">
            <div className="qa-error">
              Failed to load quiz.
              <button className="quiz-btn retry" onClick={loadQuiz}>
                Retry
              </button>
            </div>
          </section>
