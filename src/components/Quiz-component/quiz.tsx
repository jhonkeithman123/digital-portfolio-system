...
export default function QuizEditor({
  ...
  const [, setDraggingType] = useState<string | null>(null);
  ...
  const lastQuizId = useRef<string | null>(initialData?.quizId ?? null);
  ...
  function reorderOptionWithCorrect(
    length: number,
    from: number,
    to: number,
    correct: any
    ...
    if (Array.isArray(correct)) {
      return correct.map((i) => map.get(i) as number).sort((a, b) => a - b);
    } else {
  ...
  function onDragUpdate(update: any): void {
  ...
  function removePage(pageIndex: number): void {
    setPages((p) => p.filter((_, i) => i !== pageIndex));
  }

  function removeQuestion(pageId: string, qIndex: number): void {
    setPages((p) =>
      p.map((pg) =>
        pg.id !== pageId
          ? pg
  ...
  async function saveQuiz(): Promise<void> {
    if (!classroomCode) {
      showMsgRef.current("No classroom selected", "error");
      return;
    }

    const attemptsNum = Number(attemptsAllowed);
    const attempts =
      Number.isFinite(attemptsNum) && attemptsNum >= 1
        ? Math.floor(attemptsNum)
        : 1;
    const mins = parseInt(timeLimitMinutes || "0", 10);
        ...
        return {
          ...
          questions: pg.questions.map((q) => {
            ...
            if (nextType === "multiple_choice") {
              ...
              const n = parseInt(String(q.sentenceLimit ?? ""), 10);
  ...
  return (
    ...
    >
      <Header
        variant="authed"
        user={user}
        headerClass={`quiz-header ${roleClass}`}
        welcomeClass={`quiz-welcome ${roleClass}`}
      />
      {messageComponent}
      ...
      <div className="quiz-editor-shell">
        <div className="quiz-editor-card">
          <div className="qe-tools">
            <div className="q-header" style={{ alignItems: "center" }}>
